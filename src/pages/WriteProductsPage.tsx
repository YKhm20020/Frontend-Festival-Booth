	...
	return (
		<div className='flex justify-center bg-gray-100'>
			<div className='w-full max-w-xl'>
				<h1 className='text-lg font-bold text-gray-800 mt-4 mb-4 text-center'>Write Products Page</h1>
				<form className='bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4' onSubmit={handleSubmit(onSubmit)}>
					{/* タイトル入力フィールド */}
					<div>
						<label className='block text-gray-700 text-sm font-bold mb-2' htmlFor='title'>タイトル</label>
						<input
							className={`border shadow appearance-none rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline ${errors.title ? 'border-red-500' : 'border-grey-100'}`}
							id='title'
							{...register('title',{
								required: 'タイトルは必須です'
							})}
					...
					<div>
						...
						<input
							className={`border shadow appearance-none rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline ${errors.url ? 'border-red-500' : 'border-grey-100'}`}
							id='url'
							{...register('url',{
								required: 'URLは必須です',
								pattern: {
					...
					<div>
						...
						<textarea
							className='border shadow appearance-none rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline'
							id='comment'
							{...register('comment')}
